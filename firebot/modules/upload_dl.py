...
from firebot import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY
...
@fire.on(fire_on_cmd(pattern="download(?: |$)(.*)"))
@fire.on(sudo_cmd(pattern="download(?: |$)(.*)", allow_sudo=True))
...
@fire.on(fire_on_cmd(pattern=r"uploadir (.*)"))
...
@fire.on(fire_on_cmd(pattern=r"upload (.*)"))
...
@fire.on(fire_on_cmd(pattern=r"uploadas(stream|vn|all) (.*)"))
...
@borg.on(fire_on_cmd(pattern="smartdl"))
async def lul(event):
    if event.fwd_from:
        return
...
@fire.on(fire_on_cmd(pattern="zeelink"))
